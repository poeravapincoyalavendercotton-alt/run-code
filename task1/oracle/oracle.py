...
class StatTracker:
    ...
    # TODO: uncomment duplicate mappings; add ``D`` density param for complexity/uniqueness
    ...
    __INPUTS_KV = {
        ...
        # "DDR3_1Gb_x8":  {"R": _ORG_RANK, "G": 8, "B": 2, "N": 1 << 14, "C": 1 << 10, "DQ": 8},
        "DDR3_1Gb_x16": {"R": _ORG_RANK, "G": 8, "B": 2, "N": 1 << 13, "C": 1 << 10, "DQ": 16},
        # "DDR3_2Gb_x4":  {"R": _ORG_RANK, "G": 8, "B": 2, "N": 1 << 15, "C": 1 << 11, "DQ": 4},
        ...
        # "DDR3_4Gb_x8":  {"R": _ORG_RANK, "G": 8, "B": 2, "N": 1 << 16, "C": 1 << 10, "DQ": 8},
        "DDR3_4Gb_x16": {"R": _ORG_RANK, "G": 8, "B": 2, "N": 1 << 15, "C": 1 << 10, "DQ": 16},
        # "DDR3_8Gb_x4":  {"R": _ORG_RANK, "G": 8, "B": 2, "N": 1 << 17, "C": 1 << 11, "DQ": 4},
        ...
        # NOTE: DDR5
        # "DDR5_8Gb_x4":   {"R": _ORG_RANK, "G": 8, "B": 2, "N": 1 << 16,  "C": 1 << 11, "DQ": 4},
    ...
    def __execute_simulation(self, dtype: str) -> None:
        ...
        Steps
        -----
        1. Create a temporary sub-folder inside the project (task) directory.
        2. Build a GenericDDR controller with AllBank refresh and a
           CommandCounter plugin writing to ``<tmp>/cmd.count``.
        3. Drive the controller via LatencyThroughputTrace in streaming-only
           mode (no CPU trace files).
        4. Write the simulation stats YAML to ``<tmp>/stats.yaml``.
        5. Parse ``cmd.count`` (``REFab``) and the stats dict (``N_cycles`` cycles).
        6. Map measured count to ``N_refs`` and ``r_N``, ``r_G``, ``r_B`` using the
           ground-truth preset (for all-bank: ``r_N = N_refs / N`` rows per bank).

        Raises RuntimeError or ImportError if Ramulator is missing or the run
        does not yield a valid refresh window.
        """
        ...
        try:
            ...
            # Fires synthetic read/write requests as fast as the memory system
            # can accept them.  nop_counter is unused in streaming_only mode.
            ...
            # -- Run - stats accessors finalize plugins (CommandCounter CSV) --
            ...
            print(
                # f"[oracle] Ramulator done: dram_cycles={actual_cycles} "
                # f"N_refs (REFab counts)={refab_sim}{span_note}"
...
def handle_query(mode: str = "simulation", parameters: dict = None) -> dict:
    if mode == "help":
            ...
            "modes": {
                "simulation": {
                    "returns": {
                        ...
                        # "N_refs": "int - total refresh count",
        ...
        if REVEAL_SEARCH_SPACE:
            # Expose the K candidate configurations so the AI can do constrained
            # search (enumerate all configs, match by residual) instead of
            # solving the four formulae algebraically.
    ...
    if mode == "simulation":
        try:
            ...
            # NOTE: Try replacing this with others to see matching results (e.g., DDR4_2Gb_x16)
